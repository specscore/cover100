...
The run MUST be abortable at any point without losing committed work: engineers commit each package as soon as it is green (never batching to the end); units map to content-hash-named branches recorded in a per-stamp manifest so re-runs assign the same packages to the same existing branches (rebasing them onto the integration branch on adoption); the Salvage phase recovers committed-but-unintegrated branches build+test-gated; resuming with the same stamp re-collects on the existing integration branch and skips completed packages. An engineer or verifier that dies without a result (disconnect, killed agent) MUST be re-dispatched once before the unit gives up — the unit's work is durable in its worktree, so a fresh verifier re-checks committed work and a fresh engineer resumes over its predecessor's commits. A unit whose retry also dies (still no verdict) MUST be PARKED, not discarded: WIP-commit its worktree, keep its branch for the next run's Salvage. Only work an actual verifier verdict judged unsafe may be deleted.
